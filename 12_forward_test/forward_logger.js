...
function handle(strat, ev){
  ...
  if(ev.action==='close'){
    ...
    ensureCsv(f,'entry_iso,exit_iso,side,entry,exit,sl,risk,grossR,cvd,symbol');
    appendCsv(f,[op.time,iso,op.side,op.entry,exit,op.sl,op.risk,grossR!=null?grossR.toFixed(3):'',op.cvd,op.symbol]);
    delete state.open[strat]; saveState(state);
    return 'closed '+strat+' R='+(grossR!=null?grossR.toFixed(2):'?');
  }
